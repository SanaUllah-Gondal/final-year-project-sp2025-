...
class PlumberDashboardController extends GetxController {
  ...
  var userId = 0.obs; // Store user ID, not profile ID
    ...
    try {
      ...
      if (storedUserId != null) {
        ...
        debugPrint(' '
            ...
            ''
            ''
            'User ID from storage: ${userId.value}');
  ...
  Future<void> toggleOnlineStatus() async {
    try {
      ...
      final response = await _apiService.toggleOnlineStatus(
        providerType: 'plumber', // Just need the type, no user ID
...
// Load initial status from API - SIMPLIFIED
