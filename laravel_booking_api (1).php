...
1. Create new Laravel project:
   composer create-project laravel/laravel service-booking-api
...
3. Update .env file:
   ...
   DB_HOST=127.0.0.1
   DB_PORT=3306
   DB_DATABASE=service_booking_api
   DB_USERNAME=root
   DB_PASSWORD=
...
GET /api/providers/{id} - Get service provider details
PUT /api/providers/status - Update service provider status (requires auth)
...
GET /api/providers/nearby?latitude=40.7128&longitude=-74.0060&service_type=plumber&radius=10&limit=5

5. Create Booking:
POST /api/bookings
{
    "service_provider_id": 1,
    "problem_description": "Kitchen sink is leaking",
...
class DashboardController extends Controller
{
    public function customerDashboard(Request $request)
    {
        ...
        $stats = [
            ...
            'recent_bookings' => Booking::with(['serviceProvider.user'])
    ...
    public function providerDashboard(Request $request)
    {
        ...
        $stats = [
            'total_bookings' => Booking::where('service_provider_id', $serviceProvider->id)->count(),
            'pending_bookings' => Booking::where('service_provider_id', $serviceProvider->id)->where('status', 'pending')->count(),
            'completed_bookings' => Booking::where('service_provider_id', $serviceProvider->id)->where('status', 'completed')->count(),
            'total_earnings' => Booking::where('service_provider_id', $serviceProvider->id)->where('status', 'completed')->sum('final_price'),
...
class NotificationController extends Controller
{
    public function sendBookingNotification($booking, $type)
    {
        ...
        // You can integrate with Firebase, Pusher, or other services
        
        $notifications = [
            'booking_created' => [
                'title' => 'New Booking Request',
                'message' => "You have a new booking request from {$booking->customer->name}",
                'recipient' => $booking->serviceProvider->user_id,
            ],
            'booking_accepted' => [
                'title' => 'Booking Accepted',
                'message' => "Your booking has been accepted by {$booking->serviceProvider->user->name}",
        ...
        if ($notification) {
            ...
            \Log::info('Notification sent: ' . json_encode($notification));
        }
    }
}
...
class CreateBookingRequest extends FormRequest
{
    ...
    public function messages()
    {
        return [
            ...
            'service_provider_id.exists' => 'Selected service provider is not valid',
            'problem_description.required' => 'Please describe your problem',
            'problem_description.max' => 'Problem description cannot exceed 1000 characters',
            'estimated_duration.required' => 'Please estimate the duration',
            'estimated_duration.min' => 'Duration must be at least 0.5 hours',
            'estimated_duration.max' => 'Duration cannot exceed 24 hours',
            'customer_latitude.required' => 'Location is required',
            'customer_longitude.required' => 'Location is required',
    ...
    public function toArray($request)
    {
        return [
            ...
            'service_provider' => [
                ...
                'phone' => $this->serviceProvider->user->phone,
                'service_type' => $this->serviceProvider->service_type,
                'rating' => $this->serviceProvider->rating,
                'price_per_hour' => $this->serviceProvider->price_per_hour,
            ],
...
class ServiceProviderResource extends JsonResource
{
    public function toArray($request)
    {
        return [
            ...
            'estimated_arrival' => $this->when(isset($this->estimated_arrival), $this->estimated_arrival),
        ];
    }
}
...
class BookingTest extends TestCase
{
    ...
    public function test_provider_can_update_booking_status()
    {
        $provider = ServiceProvider::factory()->create();
        ...
        $response = $this->actingAs($provider->user)->putJson("/api/bookings/{$booking->id}/status", [
            'status' => 'accepted',
        ]);
...
class ServiceProviderFactory extends Factory
{
    ...
    public function definition()
    {
        return [
            ...
            'service_type' => $this->faker->randomElement(['plumber', 'electrician']),
            'rating' => $this->faker->randomFloat(2, 3, 5),
            'total_reviews' => $this->faker->numberBetween(1, 100),
            'price_per_hour' => $this->faker->randomFloat(2, 50, 150),
            'status' => 'available',
            'specialties' => $this->faker->randomElements(['Emergency repairs', 'Installation', 'Maintenance'], 2),
...
class BookingFactory extends Factory
{
    ...
    public function definition()
    {
        return [
            ...
            'estimated_duration' => $this->faker->randomFloat(2, 1, 8),
            'estimated_price' => $this->faker->randomFloat(2, 50, 500),
            'status' => 'pending',
            'customer_latitude' => $this->faker->latitude(),
            'customer_longitude' => $this->faker->longitude(),
...
1. Environment Setup:
   ...
   - Set APP_KEY and generate if needed
...
- Set up API versioning for future updates

MOBILE APP INTEGRATION:
- This API is ready for Flutter/React Native integration
- Supports real-time location tracking
- Includes push notification structure
